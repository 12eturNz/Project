...
import {
  ...
  Dumbbell, Waves, Trees, Car, Armchair, Calculator, FileText // เพิ่ม FileText icon
} from "lucide-react";
...
const initialForm = {
  ownerType: '',
  projectName: '', // เพิ่ม: ชื่อโครงการ
  description: '', // เพิ่ม: รายละเอียด
  propertyType: '',
  sellingPrice: '',
  landSize: '',
  usageArea: '',
...
const RegisterForm = () => {
  ...
  const [pricePerUnit, setPricePerUnit] = useState({ value: 0, unit: '' });
  ...
  // Logic คำนวณราคาเฉลี่ย
  useEffect(() => {
    const price = parseFloat(cleanNumber(form.sellingPrice)) || 0;
    const land = parseFloat(form.landSize) || 0;
    const area = parseFloat(form.usageArea) || 0;

    let calculatedPrice = 0;
    let unitLabel = '';

    if (form.propertyType === 'Condo' || form.propertyType === 'Apartment' || form.propertyType === 'Office') {
        if (area > 0) {
            calculatedPrice = price / area;
            unitLabel = 'บาท/ตร.ม.';
        }
    } else {
        if (land > 0) {
            calculatedPrice = price / land;
            unitLabel = 'บาท/ตร.วา';
        }
    }
    setPricePerUnit({ value: calculatedPrice, unit: unitLabel });
  }, [form.sellingPrice, form.landSize, form.usageArea, form.propertyType]);
  ...
  const validateStep = (s) => {
    ...
    if (s === 1) {
      ...
      if (!form.projectName) { e.projectName = "กรุณากรอกชื่อโครงการ"; ok = false; } // เพิ่ม validate
      if (!form.propertyType) { e.propertyType = "กรุณาเลือกประเภททรัพย์"; ok = false; }
      if (!form.sellingPrice || isNaN(parseFloat(cleanNumber(form.sellingPrice)))) { e.sellingPrice = "กรุณากรอกราคาขาย"; ok = false; }
  ...
  const handleSubmit = async (e) => {
    ...
    const newListing = {
      ...
      images: imageUrls, 
      // ใช้ชื่อโครงการและรายละเอียดจากฟอร์ม
      title: form.projectName || `${typeLabel} ใน${form.district || form.province}`,
      project: form.projectName || `${typeLabel} ใน${form.district || form.province}`,
      description: form.description || "ไม่มีรายละเอียดเพิ่มเติม",
      features: form.facilities.length > 0 ? form.facilities.join('\n') : "-",
      ...
      // ลิงก์ไปยังหน้า PropertyDetailNew (Index 0 คือรายการล่าสุด)
      link: "/Project/PropertyDetailNew/0"
    };
    ...
    try {
      ...
      alert("บันทึกไม่สำเร็จ! ข้อมูลอาจมีขนาดใหญ่เกินไป (โดยเฉพาะรูปภาพ)");
    }
  };
      ...
      <form onSubmit={handleSubmit}>
        ...
        {step === 1 && (
          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="p-6 bg-gray-50 rounded-xl">
            ...
            {/* เพิ่ม: ช่องกรอกชื่อโครงการ */}
            <FormInput 
                label="ชื่อโครงการ / ชื่อประกาศ" 
                id="projectName" 
                value={form.projectName} 
                onChange={handleChange} 
                placeholder="เช่น เดอะ พาร์คแลนด์, บ้านเดี่ยวพร้อมอยู่" 
                required
                icon={Home} 
            ...
            <div className="relative mb-5">
                ...
                {pricePerUnit.value > 0 && (
                    <div className="absolute right-4 top-[38px] text-sm font-semibold text-green-600 bg-green-50 px-2 py-1 rounded-md flex items-center gap-1">
                        <Calculator size={14} />
                        เฉลี่ย {formatNumberWithCommas(Math.round(pricePerUnit.value))} {pricePerUnit.unit}
                    </div>
                )}
            </div>

            {/* เพิ่ม: ช่องกรอกรายละเอียด */}
            <div className="mb-5 relative">
                ...
                <textarea 
                    ...
                    onChange={handleChange} 
                    className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-[#bfa074] focus:border-[#bfa074] transition duration-150 bg-white" 
                    placeholder="อธิบายจุดเด่นของบ้าน, สภาพแวดล้อม, การตกแต่ง หรือของแถม..."
                />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormInput label="ขนาดที่ดิน (ตร.วา)" id="landSize" type="number" value={form.landSize} onChange={handleChange} icon={Layers} />
            ...
            <div className="mb-4">
              ...
              <textarea name="address" id="address" rows={3} value={form.address} onChange={handleChange} className="w-full px-4 py-3 border rounded-xl shadow-sm focus:ring-2 focus:ring-[#bfa074] focus:border-[#bfa074]" placeholder="เลขที่ ซอย ถนน"></textarea>
            </div>
          </motion.div>
        )}
